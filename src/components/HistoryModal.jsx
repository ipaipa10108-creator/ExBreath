...
import { fetchGlobalHistory } from '../services/api';

export default function HistoryModal({ isOpen, onClose, history, language }) {
    const [view, setView] = useState('self'); // 'self' or 'community'
    ...
    const loadCommunity = async () => {
        ...
        if (data && Array.isArray(data)) {
            setCommunityHistory(data.slice(0, 10));
        }
                    ...
                    <div className="p-4 border-b border-[#333] flex justify-between items-center bg-[#1a1a1a]">
                        ...
                        <div className="flex bg-[#222] rounded-lg p-0.5 border border-[#333]">
                            <button
                                ...
                                className={`p-1.5 rounded-md transition-all ${view === 'self' ? 'bg-primary-gold text-black shadow-lg' : 'text-gray-500 hover:text-gray-300'}`}
                            >
                                <User size={16} title="Self" />
                            </button>
                            <button
                                onClick={() => setView('community')}
                                className={`p-1.5 rounded-md transition-all ${view === 'community' ? 'bg-primary-gold text-black shadow-lg' : 'text-gray-500 hover:text-gray-300'}`}
                            >
                                <Users size={16} title="Community" />
                            </button>
                        </div>
                    ...
                    <div className="p-6 max-h-[70vh] overflow-y-auto space-y-6">
                        {view === 'self' ? (
                            ...
                            <div className="space-y-4">
                                <h3 className="text-sm text-gray-400 mb-3 flex items-center gap-2">
                                    {t.history.communityRecords || "Community Records (Last 10 Users)"}
                                </h3>
                                {loadingCommunity ? (
                                    <div className="text-gray-500 text-center py-10">{t.history.loading}</div>
                                ) : (
                                    <div className="space-y-2">
                                        {communityHistory.length === 0 ? (
                                            <div className="text-gray-600 text-center py-10">No public records found.</div>
                                        ) : (
